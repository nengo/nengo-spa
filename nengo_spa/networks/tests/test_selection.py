...
def test_ia(Simulator, plt, seed):
    ...
    assert_allclose(sim.data[out_p][first_selection, 1:], 0.0, atol=0.05)
    assert np.all(sim.data[out_p][t > 0.95, 3] > 0.8)
    assert_allclose(sim.data[out_p][t > 0.95, :3], 0.0, atol=0.05)


def test_thresholding_array(Simulator, plt, seed):
    with nengo.Network(seed=seed) as m:
        thresholding = Thresholding(50, 4, 0.2, function=lambda x: x > 0.0)
...
def test_thresholding_array_output_shift(Simulator, plt, seed):
    ...
    assert_allclose(sim.data[p][:, 1], 0.0)
...
def test_wta(Simulator, plt, seed):
    ...
    with nengo.Network(seed=seed) as m:
        wta = WTA(50, 4, threshold=0.3, function=lambda x: x > 0.0)
